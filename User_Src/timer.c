...
{		 					 		
   ...
   //设置该引脚为复用输出功能,输出TIM3的PWM脉冲波形
	...
	TIM_OC4Init(TIM3, &TIM_OCInitStructure);  //根据指定的参数初始化外设TIMx_OCx

	TIM_OC4PreloadConfig(TIM3, TIM_OCPreload_Enable);  //CH 预装载使能
...
{		 					 	
   ...
   //设置该引脚为复用输出功能,输出TIM4 CH4的PWM脉冲波形
	...
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM2; //CH3 CH4 PWM2模式	
	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable; //比较输出使能
	TIM_OCInitStructure.TIM_Pulse = 0; //设置待装入捕获比较寄存器的脉冲值
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low; //OC 低电平有效 
	TIM_OC3Init(TIM4, &TIM_OCInitStructure);  //根据指定的参数初始化外设TIMx CH3
	TIM_OC4Init(TIM4, &TIM_OCInitStructure);  //根据指定的参数初始化外设TIMx CH4
